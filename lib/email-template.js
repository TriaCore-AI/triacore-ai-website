...
// Escape gebruikersinvoer (bv. voornaam) voor veilige rendering in HTML-mails.
...
// Resource/nieuwsbrief-titel in serif (verwijst naar de Playfair-titels op de
// site; Georgia als email-veilige basis zodat het overal rendert). Niet cursief.
export function resourceTitle(text) {
    return `<p style="margin:0 0 14px;font-family:Georgia,'Times New Roman',serif;font-size:25px;line-height:1.3;font-weight:600;color:${INK};">${text}</p>`;
}
...
export function welcomeEmail(language, opts = {}) {
    ...
    // Voornaam is gebruikersinvoer -> escapen voor de HTML-versie.
    ...
    if (nl) {
        return {
            ...
            html: emailLayout({
                preheader: 'Welkom bij de TriaCore AI community. Vanaf nu hoor je het als eerste wanneer we iets nieuws delen.',
                bodyHtml: `
                    <p style="margin:0 0 14px;font-size:18px;font-weight:700;color:${INK};">Fijn dat je erbij bent${nameHtml} 👋</p>
            ...
            text: `Fijn dat je erbij bent${nameText}. Welkom bij de TriaCore AI community. Vanaf nu hoor je het als eerste wanneer we iets nieuws delen. De AI revolutie gaat razendsnel en het is bijna onmogelijk om alles te volgen, daarom doen wij dat voor jou. Neem zeker al eens een kijkje bij wat er nu al klaarstaat, we kijken ernaar uit om je op de hoogte te houden. Bekijk de resources op ${SITE_URL}/resources. Uitschrijven kan altijd via de link onderaan elke nieuwsbrief.`,
        };
    }
    return {
        subject: 'Welcome, great to have you here',
        html: emailLayout({
            preheader: 'Welcome to the TriaCore AI community. From now on you will be the first to know when we share something new.',
            bodyHtml: `
                <p style="margin:0 0 14px;font-size:18px;font-weight:700;color:${INK};">Great to have you here${nameHtml} 👋</p>
...
export function resourceBroadcast({ title, description, thumbnail, slug, categoryLabel, readingTime, intro }) {
    const url = `${SITE_URL}/resources/${slug}`;
    const readingLabel = readingTime ? `${readingTime} min lezen` : '';
    const eyebrow = ['Nieuwe resource', categoryLabel, readingLabel].filter(Boolean).join(' &middot; ');
    const imageHtml = thumbnail
        ...
        : '';
    // Optionele pakkende openingszin boven de omschrijving (zelfde stijl, aparte alinea).
    const introHtml = intro ? `<p style="margin:0 0 14px;">${intro}</p>` : '';
    ...
    return { subject: title, previewText: description, html };
}
